...
help:
	...
	@echo "  make backtest-universal [tf=1h] [days=90] - 运行周期通用的策略回测"
	@echo "  make predict-1d    - 运行当日预测 (日线)"
	@echo "  make predict-1h    - 运行当日预测 (小时线)"
	@echo "  make predict-universal [tf=1h] [date=YYYY-MM-DD] - 运行周期通用模型预测"
...
predict-universal:
	uv run python predict.py $(if $(tf),$(tf),1h) --model models/universal_pa_smc.joblib $(if $(date),--date "$(date)",)
