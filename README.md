...
└─────────────────────────────────────────────────────────┘
                          ...
                          ↓
┌─────────────────────────────────────────────────────────┐
│ L4: 统一收益预测 (Unified Return & Risk)                  │
│ • 模型: LGBMRegressor (收益预测)                         │
│ • 输入: 技术指标 + SMC 特征 + L1 宏观数据 (1H) ⭐         │
│ • 输出: 预期收益率 → 动态仓位分配 + SMC 止盈止损            │
│ • 特性: 统一模型自动学习宏观环境对收益的影响               │
└─────────────────────────────────────────────────────────┘
```
...
- **Unified L4**: 同时作为风险管理和收益预测引擎，内隐宏观择时能力
- **多重智能**: 参数自适应 + 显式风控 + 隐式学习

## 📋 前置要求

- **Python**: 3.11+
- **工具链**: [uv](https://github.com/astral-sh/uv), make
- **系统库**: TA-Lib C Library (用于技术指标计算)
...
make optimize-all # 优化所有模型 (L2 + L3)
```
...
make backtest-vbt-full  # 完整 90 天 + 10万本金回测
...
- **预期收益预测**: 统一模型预测未来 1H 的预期收益率
- **宏观环境感知**: 整合 L1 宏观数据 (SPY, VIXY, TLT), 在统一模型中隐式学习市场风险 ⭐
...
algo-trade/
├── config/
│   ├── settings.py          # 全局配置 (标的池、阈值等)
│   └── dynamic_params.py    # L5 动态参数管理
├── data/
...
├── strategies/
│   ├── engine.py            # 策略引擎
│   ├── meta_strategy.py     # L5 元策略模型
│   └── smc_rules.py         # SMC 止盈止损规则
├── training/
│   ├── trainer.py           # 模型训练器
│   └── robust_trainer.py    # 稳健训练 (Purged CV)
├── models/artifacts/        # 训练好的模型
│   ├── l2_stock_selection.joblib
│   ├── l3_execution.joblib
│   ├── unified_return_predictor.joblib  # L1+L4 统一模型
│   └── l5_meta_strategy.joblib
├── scripts/
│   ├── train_l2.py          # L2 训练脚本
│   ├── train_l3.py          # L3 训练脚本
│   ├── train_l4.py          # L4 统一收益模型训练
│   ├── train_l5.py          # L5 元策略训练
│   ├── backtest_*.py        # 分层回测脚本
│   └── optimize_*.py        # Optuna 超参数优化
├── web/
...
| L4   | 小时线 (统一) | 2023-12-31 ~ 2024-12-31 | 1 年统一收益模型   |
